...
//    cout << endl;
    ...
    for (int i = 0; i < machineNum; i++)
    {
        if (low[i] * fixTime[i] >= (output[i] - low[i]) / decrease[i] * output[i] /2 || low[i] >= 50)
            considerFix[i] = 0;
        else
            considerFix[i] = 1;
    }
...
//        cout << endl;
        ...
        for (int i = 0; i < machineNum; i++)
        {
            ...
            if (machineJobClearTime[machineRank[i]] == 0)
            {
                ...
                if(considerFix[machineRank[i]] == 1 && init[machineRank[i]] <= low[machineRank[i]] + 20 && workerNum > 0)
                {
